...
namespace ArgParser.Flavors.Test
{
    public class GitParameterRepository_Should
    {
        ...
        public void Correctly_Return_Parameters_For_A_Registered_Flavor()
        {
            ...
            {
                for (var i = 0; i < 4; i++) repo.AddParameter(name, new BooleanSwitch());
                for (var i = 0; i < 3; i++) repo.AddParameter(name, new Positional());
                for (var i = 0; i < 2; i++) repo.AddParameter(name, new SingleValueSwitch());
                for (var i = 0; i < 1; i++) repo.AddParameter(name, new ValuesSwitch());
            }
